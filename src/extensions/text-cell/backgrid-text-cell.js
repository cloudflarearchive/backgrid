...
(function (window, $, _, Backbone, Backgrid)  {
   ...
   */
  var TextareaEditor = Backgrid.Extension.TextareaEditor = Backgrid.DivCellEditor.extend({
    ...
    template: _.template('<form><div class="modal-header"><button class="close" data-dismiss="modal">&times;</button><h3><%= column.get("label") %></h3></div><div class="modal-body"><textarea cols="<%= cols %>" rows="<%= rows %>"><%= content %></textarea></div><div class="modal-footer"><input class="btn" type="submit" value="Save"/></div></form>'),
    ...
    */
    save: function (e) {
      ...
      if (_.isUndefined(content) || !this.model.set(this.column.get("name"), content)) {
        this.trigger("error");
      }
      else {
        this.$el.modal("hide");
    ...
    */
    cancel: function (e) {
      ...
      // Dirty
      if (content !== this.model.get(this.column.get("name")) &&
